; ====================================================================
; Código Assembly gerado automaticamente
; Arquitetura: AVR ATmega328P (Arduino Uno)
; Compilador: RA4_Compiladores - Fase 4
; ====================================================================

.section .text
.global main

main:
    ; Inicializar stack pointer
    ldi r16, lo8(0x08FF)   ; RAMEND low byte
    out 0x3D, r16          ; SPL
    ldi r16, hi8(0x08FF)   ; RAMEND high byte
    out 0x3E, r16          ; SPH

    ; Inicializar UART (115200 baud @ 16MHz)
    rcall uart_init

    ; ==== INÍCIO DO CÓDIGO GERADO ====

    ; TAC linha 1: t0 = 100
    ldi r16, 100   ; Constante 100 (low byte)
    ldi r17, 0  ; Constante 100 (high byte)

    ; TAC linha 1: X_VAL = t0
    mov r18, r16   ; X_VAL = t0
    mov r19, r17

    ; Liberando registradores de variáveis mortas: t0

    ; TAC linha 2: t1 = 100
    ldi r20, 100   ; Constante 100 (low byte)
    ldi r21, 0  ; Constante 100 (high byte)

    ; TAC linha 2: TERM1 = t1
    mov r22, r20   ; TERM1 = t1
    mov r23, r21

    ; Liberando registradores de variáveis mortas: t1

    ; TAC linha 3: t2 = 1
    ldi r2, 1   ; Constante 1 (low byte)
    ldi r3, 0  ; Constante 1 (high byte)

    ; TAC linha 3: COUNTER = t2
    mov r4, r2   ; COUNTER = t2
    mov r5, r3

    ; Liberando registradores de variáveis mortas: t2

L0:

    ; TAC linha 4: t3 = 1
    ldi r6, 1   ; Constante 1 (low byte)
    ldi r7, 0  ; Constante 1 (high byte)

    ; TAC linha 4: t4 = COUNTER <= t3
    ; Comparação 16-bit menor ou igual (unsigned)
    ; Implementado como: B >= A (operandos trocados)
    cp r6, r4      ; Compare B with A (reversed)
    cpc r7, r5
    ldi r8, 0               ; Assume false
    ldi r9, 0
    brlo skip_le_4               ; If B < A, skip
    ldi r8, 1               ; Set true (B >= A, i.e., A <= B)
skip_le_4:

    ; Liberando registradores de variáveis mortas: t3

    ; TAC linha 4: ifFalse t4 goto L1
    ; Check if t4 == 0 (false)
    ldi r24, 0                  ; Zero constant for comparison
    ldi r25, 0
    cp r8, r24         ; Compare low byte with 0
    cpc r9, r25       ; Compare high byte with carry
    breq L1               ; Branch if equal (condition is false)

    ; Liberando registradores de variáveis mortas: t4

    ; TAC linha 4: t5 = X_VAL * X_VAL
    ; Multiplicação 16-bit com re-normalização de escala
    ; Preparar parâmetros para mul16
    mov r18, r18
    mov r19, r19
    mov r20, r18
    mov r21, r19
    rcall mul16              ; R24:R25 = op1 * op2 (scaled²)
    ; Re-normalize: divide by 100 to restore 100x scaling
    mov r18, r24
    mov r19, r25
    ldi r20, 100             ; Divide by scale factor
    ldi r21, 0
    rcall div16              ; R24:R25 = result (scaled¹)
    mov r10, r24
    mov r11, r25

    ; Liberando registradores de variáveis mortas: X_VAL

    ; TAC linha 4: X_SQUARE = t5
    mov r12, r10   ; X_SQUARE = t5
    mov r13, r11

    ; Liberando registradores de variáveis mortas: t5

    ; TAC linha 4: t6 = 200
    ldi r14, 200   ; Constante 200 (low byte)
    ldi r15, 0  ; Constante 200 (high byte)

    ; TAC linha 4: FACT_2 = t6
    mov r24, r14   ; FACT_2 = t6
    mov r25, r15

    ; Liberando registradores de variáveis mortas: t6

    ; TAC linha 4: t7 = X_SQUARE | FACT_2
    ; Real division: (op1 * 100) / op2 for precision
    ; Step 1: Multiply dividend by scale factor (100)
    mov r18, r12      ; Dividendo
    mov r19, r13
    ldi r20, 100             ; Scale factor (100)
    ldi r21, 0
    rcall mul16              ; R24:R25 = op1 * 100
    ; Step 2: Divide scaled dividend by divisor
    mov r18, r24             ; Move result to dividend registers
    mov r19, r25
    mov r20, r24      ; Divisor
    mov r21, r25
    rcall div16              ; R24:R25 = (op1 * 100) / op2
    mov r16, r24      ; Copy result
    mov r17, r25

    ; Liberando registradores de variáveis mortas: FACT_2

    ; TAC linha 4: TEMP2 = t7
    mov r20, r16   ; TEMP2 = t7
    mov r21, r17

    ; Liberando registradores de variáveis mortas: t7

    ; TAC linha 4: t8 = 0
    ldi r2, 0   ; Constante 0 (low byte)
    ldi r3, 0  ; Constante 0 (high byte)

    ; TAC linha 4: t9 = t8 - TEMP2
    ; Subtração 16-bit: t9 = t8 - TEMP2
    sub r2, r20   ; Low byte com borrow
    sbc r3, r21 ; High byte com borrow
    mov r6, r2   ; Resultado em t9
    mov r7, r3

    ; Liberando registradores de variáveis mortas: TEMP2, t8

    ; TAC linha 4: TERM2 = t9
    mov r8, r6   ; TERM2 = t9
    mov r9, r7

    ; Liberando registradores de variáveis mortas: t9

    ; TAC linha 4: t10 = X_SQUARE * X_SQUARE
    ; Multiplicação 16-bit com re-normalização de escala
    ; Preparar parâmetros para mul16
    mov r18, r12
    mov r19, r13
    mov r20, r12
    mov r21, r13
    rcall mul16              ; R24:R25 = op1 * op2 (scaled²)
    ; Re-normalize: divide by 100 to restore 100x scaling
    mov r18, r24
    mov r19, r25
    ldi r20, 100             ; Divide by scale factor
    ldi r21, 0
    rcall div16              ; R24:R25 = result (scaled¹)
    mov r18, r24
    mov r19, r25

    ; TAC linha 4: X_FOURTH = t10
    mov r10, r18   ; X_FOURTH = t10
    mov r11, r19

    ; Liberando registradores de variáveis mortas: t10

    ; TAC linha 4: t11 = 400
    ldi r14, 144   ; Constante 400 (low byte)
    ldi r15, 1  ; Constante 400 (high byte)

    ; TAC linha 4: t12 = 300
    ldi r24, 44   ; Constante 300 (low byte)
    ldi r25, 1  ; Constante 300 (high byte)

    ; TAC linha 4: t13 = t11 * t12
    ; Multiplicação 16-bit com re-normalização de escala
    ; Preparar parâmetros para mul16
    mov r18, r14
    mov r19, r15
    mov r20, r24
    mov r21, r25
    rcall mul16              ; R24:R25 = op1 * op2 (scaled²)
    ; Re-normalize: divide by 100 to restore 100x scaling
    mov r18, r24
    mov r19, r25
    ldi r20, 100             ; Divide by scale factor
    ldi r21, 0
    rcall div16              ; R24:R25 = result (scaled¹)
    mov r16, r24
    mov r17, r25

    ; Liberando registradores de variáveis mortas: t11, t12

    ; TAC linha 4: TEMP4A = t13
    mov r20, r16   ; TEMP4A = t13
    mov r21, r17

    ; Liberando registradores de variáveis mortas: t13

    ; TAC linha 4: t14 = 200
    ldi r2, 200   ; Constante 200 (low byte)
    ldi r3, 0  ; Constante 200 (high byte)

    ; TAC linha 4: t15 = TEMP4A * t14
    ; Multiplicação 16-bit com re-normalização de escala
    ; Preparar parâmetros para mul16
    mov r18, r20
    mov r19, r21
    mov r20, r2
    mov r21, r3
    rcall mul16              ; R24:R25 = op1 * op2 (scaled²)
    ; Re-normalize: divide by 100 to restore 100x scaling
    mov r18, r24
    mov r19, r25
    ldi r20, 100             ; Divide by scale factor
    ldi r21, 0
    rcall div16              ; R24:R25 = result (scaled¹)
    mov r6, r24
    mov r7, r25

    ; Liberando registradores de variáveis mortas: TEMP4A, t14

    ; TAC linha 4: TEMP4B = t15
    mov r18, r6   ; TEMP4B = t15
    mov r19, r7

    ; Liberando registradores de variáveis mortas: t15

    ; TAC linha 4: t16 = 100
    ldi r14, 100   ; Constante 100 (low byte)
    ldi r15, 0  ; Constante 100 (high byte)

    ; TAC linha 4: t17 = TEMP4B * t16
    ; Multiplicação 16-bit com re-normalização de escala
    ; Preparar parâmetros para mul16
    mov r18, r18
    mov r19, r19
    mov r20, r14
    mov r21, r15
    rcall mul16              ; R24:R25 = op1 * op2 (scaled²)
    ; Re-normalize: divide by 100 to restore 100x scaling
    mov r18, r24
    mov r19, r25
    ldi r20, 100             ; Divide by scale factor
    ldi r21, 0
    rcall div16              ; R24:R25 = result (scaled¹)
    mov r24, r24
    mov r25, r25

    ; Liberando registradores de variáveis mortas: TEMP4B, t16

    ; TAC linha 4: FACT_4 = t17
    mov r16, r24   ; FACT_4 = t17
    mov r17, r25

    ; Liberando registradores de variáveis mortas: t17

    ; TAC linha 4: t18 = X_FOURTH | FACT_4
    ; Real division: (op1 * 100) / op2 for precision
    ; Step 1: Multiply dividend by scale factor (100)
    mov r18, r10      ; Dividendo
    mov r19, r11
    ldi r20, 100             ; Scale factor (100)
    ldi r21, 0
    rcall mul16              ; R24:R25 = op1 * 100
    ; Step 2: Divide scaled dividend by divisor
    mov r18, r24             ; Move result to dividend registers
    mov r19, r25
    mov r20, r16      ; Divisor
    mov r21, r17
    rcall div16              ; R24:R25 = (op1 * 100) / op2
    mov r20, r24      ; Copy result
    mov r21, r25

    ; Liberando registradores de variáveis mortas: FACT_4

    ; TAC linha 4: TERM3 = t18
    mov r2, r20   ; TERM3 = t18
    mov r3, r21

    ; Liberando registradores de variáveis mortas: t18

    ; TAC linha 4: t19 = X_FOURTH * X_SQUARE
    ; Multiplicação 16-bit com re-normalização de escala
    ; Preparar parâmetros para mul16
    mov r18, r10
    mov r19, r11
    mov r20, r12
    mov r21, r13
    rcall mul16              ; R24:R25 = op1 * op2 (scaled²)
    ; Re-normalize: divide by 100 to restore 100x scaling
    mov r18, r24
    mov r19, r25
    ldi r20, 100             ; Divide by scale factor
    ldi r21, 0
    rcall div16              ; R24:R25 = result (scaled¹)
    mov r6, r24
    mov r7, r25

    ; Liberando registradores de variáveis mortas: X_SQUARE, X_FOURTH

    ; TAC linha 4: X_SIXTH = t19
    mov r18, r6   ; X_SIXTH = t19
    mov r19, r7

    ; Liberando registradores de variáveis mortas: t19

    ; TAC linha 4: t20 = 600
    ldi r14, 88   ; Constante 600 (low byte)
    ldi r15, 2  ; Constante 600 (high byte)

    ; TAC linha 4: t21 = 500
    ldi r24, 244   ; Constante 500 (low byte)
    ldi r25, 1  ; Constante 500 (high byte)

    ; TAC linha 4: t22 = t20 * t21
    ; Multiplicação 16-bit com re-normalização de escala
    ; Preparar parâmetros para mul16
    mov r18, r14
    mov r19, r15
    mov r20, r24
    mov r21, r25
    rcall mul16              ; R24:R25 = op1 * op2 (scaled²)
    ; Re-normalize: divide by 100 to restore 100x scaling
    mov r18, r24
    mov r19, r25
    ldi r20, 100             ; Divide by scale factor
    ldi r21, 0
    rcall div16              ; R24:R25 = result (scaled¹)
    mov r16, r24
    mov r17, r25

    ; Liberando registradores de variáveis mortas: t20, t21

    ; TAC linha 4: TEMP6A = t22
    mov r20, r16   ; TEMP6A = t22
    mov r21, r17

    ; Liberando registradores de variáveis mortas: t22

    ; TAC linha 4: t23 = 400
    ldi r12, 144   ; Constante 400 (low byte)
    ldi r13, 1  ; Constante 400 (high byte)

    ; TAC linha 4: t24 = TEMP6A * t23
    ; Multiplicação 16-bit com re-normalização de escala
    ; Preparar parâmetros para mul16
    mov r18, r20
    mov r19, r21
    mov r20, r12
    mov r21, r13
    rcall mul16              ; R24:R25 = op1 * op2 (scaled²)
    ; Re-normalize: divide by 100 to restore 100x scaling
    mov r18, r24
    mov r19, r25
    ldi r20, 100             ; Divide by scale factor
    ldi r21, 0
    rcall div16              ; R24:R25 = result (scaled¹)
    mov r10, r24
    mov r11, r25

    ; Liberando registradores de variáveis mortas: TEMP6A, t23

    ; TAC linha 4: TEMP6B = t24
    mov r6, r10   ; TEMP6B = t24
    mov r7, r11

    ; Liberando registradores de variáveis mortas: t24

    ; TAC linha 4: t25 = 300
    ldi r14, 44   ; Constante 300 (low byte)
    ldi r15, 1  ; Constante 300 (high byte)

    ; TAC linha 4: t26 = TEMP6B * t25
    ; Multiplicação 16-bit com re-normalização de escala
    ; Preparar parâmetros para mul16
    mov r18, r6
    mov r19, r7
    mov r20, r14
    mov r21, r15
    rcall mul16              ; R24:R25 = op1 * op2 (scaled²)
    ; Re-normalize: divide by 100 to restore 100x scaling
    mov r18, r24
    mov r19, r25
    ldi r20, 100             ; Divide by scale factor
    ldi r21, 0
    rcall div16              ; R24:R25 = result (scaled¹)
    mov r24, r24
    mov r25, r25

    ; Liberando registradores de variáveis mortas: TEMP6B, t25

    ; TAC linha 4: TEMP6C = t26
    mov r16, r24   ; TEMP6C = t26
    mov r17, r25

    ; Liberando registradores de variáveis mortas: t26

    ; TAC linha 4: t27 = 200
    ldi r20, 200   ; Constante 200 (low byte)
    ldi r21, 0  ; Constante 200 (high byte)

    ; TAC linha 4: t28 = TEMP6C * t27
    ; Multiplicação 16-bit com re-normalização de escala
    ; Preparar parâmetros para mul16
    mov r18, r16
    mov r19, r17
    mov r20, r20
    mov r21, r21
    rcall mul16              ; R24:R25 = op1 * op2 (scaled²)
    ; Re-normalize: divide by 100 to restore 100x scaling
    mov r18, r24
    mov r19, r25
    ldi r20, 100             ; Divide by scale factor
    ldi r21, 0
    rcall div16              ; R24:R25 = result (scaled¹)
    mov r12, r24
    mov r13, r25

    ; Liberando registradores de variáveis mortas: TEMP6C, t27

    ; TAC linha 4: TEMP6D = t28
    mov r10, r12   ; TEMP6D = t28
    mov r11, r13

    ; Liberando registradores de variáveis mortas: t28

    ; TAC linha 4: t29 = 100
    ldi r6, 100   ; Constante 100 (low byte)
    ldi r7, 0  ; Constante 100 (high byte)

    ; TAC linha 4: t30 = TEMP6D * t29
    ; Multiplicação 16-bit com re-normalização de escala
    ; Preparar parâmetros para mul16
    mov r18, r10
    mov r19, r11
    mov r20, r6
    mov r21, r7
    rcall mul16              ; R24:R25 = op1 * op2 (scaled²)
    ; Re-normalize: divide by 100 to restore 100x scaling
    mov r18, r24
    mov r19, r25
    ldi r20, 100             ; Divide by scale factor
    ldi r21, 0
    rcall div16              ; R24:R25 = result (scaled¹)
    mov r14, r24
    mov r15, r25

    ; Liberando registradores de variáveis mortas: TEMP6D, t29

    ; TAC linha 4: FACT_6 = t30
    mov r24, r14   ; FACT_6 = t30
    mov r25, r15

    ; Liberando registradores de variáveis mortas: t30

    ; TAC linha 4: t31 = X_SIXTH | FACT_6
    ; Real division: (op1 * 100) / op2 for precision
    ; Step 1: Multiply dividend by scale factor (100)
    mov r18, r18      ; Dividendo
    mov r19, r19
    ldi r20, 100             ; Scale factor (100)
    ldi r21, 0
    rcall mul16              ; R24:R25 = op1 * 100
    ; Step 2: Divide scaled dividend by divisor
    mov r18, r24             ; Move result to dividend registers
    mov r19, r25
    mov r20, r24      ; Divisor
    mov r21, r25
    rcall div16              ; R24:R25 = (op1 * 100) / op2
    mov r16, r24      ; Copy result
    mov r17, r25

    ; Liberando registradores de variáveis mortas: X_SIXTH, FACT_6

    ; TAC linha 4: TEMP8 = t31
    mov r20, r16   ; TEMP8 = t31
    mov r21, r17

    ; Liberando registradores de variáveis mortas: t31

    ; TAC linha 4: t32 = 0
    ldi r12, 0   ; Constante 0 (low byte)
    ldi r13, 0  ; Constante 0 (high byte)

    ; TAC linha 4: t33 = t32 - TEMP8
    ; Subtração 16-bit: t33 = t32 - TEMP8
    sub r12, r20   ; Low byte com borrow
    sbc r13, r21 ; High byte com borrow
    mov r10, r12   ; Resultado em t33
    mov r11, r13

    ; Liberando registradores de variáveis mortas: TEMP8, t32

    ; TAC linha 4: TERM4 = t33
    mov r6, r10   ; TERM4 = t33
    mov r7, r11

    ; Liberando registradores de variáveis mortas: t33

    ; TAC linha 4: t34 = TERM1 + TERM2
    ; Soma 16-bit: t34 = TERM1 + TERM2
    add r22, r8   ; Low byte com carry
    adc r23, r9 ; High byte com carry
    mov r14, r22   ; Resultado em t34
    mov r15, r23

    ; Liberando registradores de variáveis mortas: TERM1, TERM2

    ; TAC linha 4: SUM12 = t34
    mov r18, r14   ; SUM12 = t34
    mov r19, r15

    ; Liberando registradores de variáveis mortas: t34

    ; TAC linha 4: t35 = SUM12 + TERM3
    ; Soma 16-bit: t35 = SUM12 + TERM3
    add r18, r2   ; Low byte com carry
    adc r19, r3 ; High byte com carry
    mov r24, r18   ; Resultado em t35
    mov r25, r19

    ; Liberando registradores de variáveis mortas: TERM3, SUM12

    ; TAC linha 4: SUM123 = t35
    mov r16, r24   ; SUM123 = t35
    mov r17, r25

    ; Liberando registradores de variáveis mortas: t35

    ; TAC linha 4: t36 = SUM123 + TERM4
    ; Soma 16-bit: t36 = SUM123 + TERM4
    add r16, r6   ; Low byte com carry
    adc r17, r7 ; High byte com carry
    mov r20, r16   ; Resultado em t36
    mov r21, r17

    ; Liberando registradores de variáveis mortas: TERM4, SUM123

    ; TAC linha 4: RESULT_COS = t36
    mov r12, r20   ; RESULT_COS = t36
    mov r13, r21

    ; Liberando registradores de variáveis mortas: t36

    ; TAC linha 4: FINAL_COS = RESULT_COS
    mov r10, r12   ; FINAL_COS = RESULT_COS
    mov r11, r13

    ; Liberando registradores de variáveis mortas: RESULT_COS, FINAL_COS

    ; TAC linha 4: t37 = 1
    ldi r22, 1   ; Constante 1 (low byte)
    ldi r23, 0  ; Constante 1 (high byte)

    ; TAC linha 4: t38 = COUNTER + t37
    ; Soma 16-bit: t38 = COUNTER + t37
    add r4, r22   ; Low byte com carry
    adc r5, r23 ; High byte com carry
    mov r8, r4   ; Resultado em t38
    mov r9, r5

    ; Liberando registradores de variáveis mortas: t37

    ; TAC linha 4: COUNTER = t38
    mov r4, r8   ; COUNTER = t38
    mov r5, r9

    ; Liberando registradores de variáveis mortas: COUNTER, t38

    ; TAC linha 4: goto L0
    rjmp L0   ; Salto relativo para L0

L1:

    ; ==== FIM DO CÓDIGO GERADO ====

; ====================================================================
; mul16: Multiplicação 16-bit × 16-bit = 16-bit (unsigned)
; Entrada: R18:R19 (op1), R20:R21 (op2)
; Saída: R24:R25 (resultado)
; Usa: R0, R1, R22, R23
; ====================================================================
mul16:
    ; Salvar registradores que serão modificados
    push r22
    push r23

    ; Zerar acumulador resultado
    clr r24
    clr r25

    ; Produto parcial 1: AL × BL (contribui totalmente)
    mul r18, r20      ; R1:R0 = AL × BL
    mov r24, r0       ; Byte baixo do resultado
    mov r22, r1       ; Byte alto vai para temporário

    ; Produto parcial 2: AL × BH (contribui byte baixo para byte alto do resultado)
    mul r18, r21      ; R1:R0 = AL × BH
    add r22, r0       ; Somar byte baixo ao acumulador
    adc r25, r1       ; Somar byte alto com carry

    ; Produto parcial 3: AH × BL (contribui byte baixo para byte alto do resultado)
    mul r19, r20      ; R1:R0 = AH × BL
    add r22, r0       ; Somar byte baixo ao acumulador
    adc r25, r1       ; Somar byte alto com carry

    ; Produto parcial 4: AH × BH (descartado - overflow além de 16 bits)
    ; Não precisamos calcular pois descartamos resultado > 16 bits

    ; Mover acumulador para resultado final
    mov r25, r22      ; Byte alto do resultado

    ; Restaurar registradores
    pop r23
    pop r22

    ; Limpar R0 e R1 (boa prática após MUL)
    clr r1

    ret

; ====================================================================
; div16: Divisão 16-bit ÷ 16-bit = quociente e resto (unsigned)
; Algoritmo: Restoring shift-subtract (17 iterações, resto corrigido)
; Entrada: R18:R19 (dividendo), R20:R21 (divisor)
; Saída: R24:R25 (quociente), R22:R23 (resto)
; Usa: R16 (contador de loop)
; Ciclos: ~232
; ====================================================================
div16:
    push r16              ; Salvar registrador usado

    ; Verificar divisão por zero
    cp      r20, r1       ; Comparar divisor low com 0
    cpc     r21, r1       ; Comparar divisor high com 0
    breq    div16_by_zero ; Se zero, pular para tratamento de erro

    ; Inicializar resto = 0
    clr     r22           ; Resto low = 0
    clr     r23           ; Resto high = 0

    ; Inicializar contador de loop (16 iterações para divisão 16-bit)
    ldi     r16, 16

div16_loop:
    ; Deslocar dividendo/quociente para esquerda
    lsl     r18           ; Logical shift left (LSB=0, no carry dependency)
    rol     r19           ; Shift dividend/quotient high

    ; Deslocar bit MSB do dividendo para o resto
    rol     r22           ; Shift into remainder low
    rol     r23           ; Shift into remainder high

    ; Comparar resto com divisor
    cp      r22, r20      ; Compare remainder with divisor (low)
    cpc     r23, r21      ; Compare remainder with divisor (high)
    brcs    div16_skip    ; If remainder < divisor, skip subtraction

    ; Resto >= divisor: subtrair divisor do resto
    sub     r22, r20      ; Subtract divisor from remainder (low)
    sbc     r23, r21      ; Subtract divisor from remainder (high)
    inc     r18           ; Set quotient LSB bit to 1

div16_skip:
    dec     r16           ; Decrementar contador
    brne    div16_loop    ; Loop se não terminou

    ; Mover quociente para registradores de saída
    mov     r24, r18      ; Quotient to output (low)
    mov     r25, r19      ; Quotient to output (high)

    ; Resto já está em R22:R23 (correto)

    pop r16
    ret

div16_by_zero:
    ; Retornar valores de erro
    ldi     r24, 0xFF     ; Quociente = 0xFFFF (indicador de erro)
    ldi     r25, 0xFF
    mov     r22, r18      ; Resto = dividendo original (low)
    mov     r23, r19      ; Resto = dividendo original (high)
    pop r16
    ret

; ====================================================================
; ROTINAS UART (115200 baud @ 16MHz)
; ====================================================================

uart_init:
    ldi r16, 0
    sts 0xC5, r16        ; UBRR0H = 0
    ldi r16, 8
    sts 0xC4, r16        ; UBRR0L = 8 (115200 baud @ 16MHz)
    ldi r16, (1 << 3)    ; TXEN0
    sts 0xC1, r16        ; UCSR0B
    ldi r16, (1 << 2) | (1 << 1)
    sts 0xC2, r16        ; UCSR0C (8N1)
    ret

uart_transmit:
    push r25
uart_wait:
    lds r25, 0xC0        ; UCSR0A
    sbrs r25, 5          ; Wait for UDRE0
    rjmp uart_wait
    sts 0xC6, r24        ; UDR0 = data
    pop r25
    ret

; ====================================================================
; send_number_16bit: Envia número 16-bit como decimal via UART
; Entrada: R24:R25 (número 16-bit, 0-65535)
; Saída: Nenhuma (envia via UART)
; ====================================================================
send_number_16bit:
    push r20
    push r21
    push r22
    push r23
    push r16
    push r17
    push r18
    push r19

    mov r20, r24
    mov r21, r25

    ; Dezenas de milhares (10000)
    ldi r16, lo8(10000)
    ldi r17, hi8(10000)
    clr r18

div_10000:
    cp r20, r16
    cpc r21, r17
    brlo div_1000
    sub r20, r16
    sbc r21, r17
    inc r18
    rjmp div_10000

div_1000:
    ; Imprimir dezena de milhares se != 0
    cpi r18, 0
    breq skip_10000
    mov r24, r18
    subi r24, -48
    rcall uart_transmit
skip_10000:

    ; Milhares (1000)
    ldi r16, lo8(1000)
    ldi r17, hi8(1000)
    clr r18

div_1000_loop:
    cp r20, r16
    cpc r21, r17
    brlo div_100
    sub r20, r16
    sbc r21, r17
    inc r18
    rjmp div_1000_loop

div_100:
    ; Imprimir milhares
    mov r24, r18
    subi r24, -48
    rcall uart_transmit

    ; Centenas (100)
    ldi r16, 100
    clr r17
    clr r18

div_100_loop:
    cp r20, r16
    cpc r21, r17
    brlo div_10
    sub r20, r16
    sbc r21, r17
    inc r18
    rjmp div_100_loop

div_10:
    ; Imprimir centenas
    mov r24, r18
    subi r24, -48
    rcall uart_transmit

    ; Dezenas (10)
    ldi r16, 10
    clr r17
    clr r18

div_10_loop:
    cp r20, r16
    cpc r21, r17
    brlo print_units
    sub r20, r16
    sbc r21, r17
    inc r18
    rjmp div_10_loop

print_units:
    ; Imprimir dezenas
    mov r24, r18
    subi r24, -48
    rcall uart_transmit

    ; Imprimir unidades
    mov r24, r20
    subi r24, -48
    rcall uart_transmit

    pop r19
    pop r18
    pop r17
    pop r16
    pop r23
    pop r22
    pop r21
    pop r20
    ret

fim:
    rjmp fim   ; Loop infinito

; ====================================================================
; Fim do programa
; ====================================================================