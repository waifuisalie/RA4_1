; ====================================================================
; TESTE 12 DEBUG: Real Division with Scaling (1 | 2 = 500)
; ====================================================================
; Valida: Divisão real escalada por 1000 COM LOGGING
; Operação: 1 | 2 deve resultar em 500 (representa 0.500)
; Saída esperada via UART 115200: "A01F4 B05:00 C00:00 500"
; ====================================================================
...
main:
    ...
    ; ========== DEBUG POINT A: Result of div_scaled ==========
    push r24
    push r25
    ldi r24, 'A'
    rcall uart_transmit
    ...
    ; Send high byte as hex
    push r24
    mov r24, r25
    rcall send_hex_byte
    pop r24

    ; Send low byte as hex
    rcall send_hex_byte
    ...
    ; ========== DEBUG POINT B: Result of 500/100 ==========
    push r24
    push r22
    ldi r24, 'B'
    rcall uart_transmit
    ...
    ; ========== DEBUG POINT C: Result of remainder/10 ==========
    push r24
    push r22
    ldi r24, 'C'
    rcall uart_transmit
...
; ====================================================================
; ROTINA: send_hex_byte - Envia byte como 2 dígitos hex
; ====================================================================
; Entrada: R24 (byte to send)
; Saída: Nenhuma (sends via UART)
; ====================================================================
send_hex_byte:
    push r16
    push r17
    ...
    andi r16, 0x0F
    cpi r16, 10
    brlo send_hex_high_0_9
    subi r16, -55          ; 'A' - 10
    rjmp send_hex_high_done
send_hex_high_0_9:
    subi r16, -48          ; '0'
send_hex_high_done:
    mov r17, r24
    ...
    andi r16, 0x0F
    cpi r16, 10
    brlo send_hex_low_0_9
    subi r16, -55          ; 'A' - 10
    rjmp send_hex_low_done
send_hex_low_0_9:
    subi r16, -48          ; '0'
send_hex_low_done:
    mov r24, r16
