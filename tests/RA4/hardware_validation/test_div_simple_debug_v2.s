...
main:
    ...
    ; DEBUG: Show A value (should be 03E8 = 1000)
    ldi r24, 'A'
    rcall uart_transmit
    ldi r24, '='
    rcall uart_transmit
    ...
    ; DEBUG: Show B value (should be 07D0 = 2000)
    ldi r24, 'B'
    rcall uart_transmit
    ldi r24, '='
    rcall uart_transmit
    ...
    ; TAC linha 3: t2 = A | B
    ; Divisão real escalada: (A * 1000) / B
    ; Preparar parâmetros para div_scaled
    mov r18, r18      ; Dividendo (low)
    mov r19, r19     ; Dividendo (high)
    mov r20, r22      ; Divisor (low)
    mov r21, r23     ; Divisor (high)
    rcall div_scaled         ; Chamar rotina (resultado escalado em R24:R25)

    ; DEBUG: Show division result (should be 01F4 = 500)
    push r24
    push r25
    ldi r24, 'D'
    rcall uart_transmit
    ...
    mov r16, r24      ; Copiar resultado escalado
    mov r17, r25
    ...
    ; DEBUG: Show FINAL value (should be 01F4 = 500)
    ldi r24, 'F'
    rcall uart_transmit
    ldi r24, '='
    rcall uart_transmit
...
div_scaled:
    ...
    ; DEBUG 1: Show input parameters (dividendo and divisor)
    push r18
    push r19
    push r20
    push r21
    ...
    ; DEBUG 2: Show mul16 result (should be 4240 hex = 16960 decimal)
    push r24
    push r25
    ldi r24, '2'
    rcall uart_transmit
    ...
    ; DEBUG 3: Show R18:R19 after move (should match mul16 result)
    push r18
    push r19
    ldi r24, '3'
    rcall uart_transmit
    ...
    ; DEBUG 4: Show divisor after restoring from stack (should be 07D0)
    push r20
    push r21
    ldi r24, '4'
    rcall uart_transmit
    ...
    ; DEBUG 5: Show div16 result (should be 01F4 = 500)
    push r24
    push r25
    ldi r24, '5'
    rcall uart_transmit
...
; ====================================================================
; send_hex_word: Envia palavra 16-bit como 4 dígitos hexadecimais
...
; ====================================================================
; send_hex_byte: Envia byte como 2 dígitos hexadecimais
; Entrada: R24 (byte)
; Saída: Nenhuma (envia via UART)
; ====================================================================
send_hex_byte:
