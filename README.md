...
1. **TAC Original (JSON):** `outputs/RA4/tac_instructions.json`
2. **TAC Original (Markdown):** `outputs/RA4/tac_output.md`
3. **TAC Otimizado (JSON):** `outputs/RA4/tac_otimizado.json`
4. **TAC Otimizado (Markdown):** `outputs/RA4/tac_otimizado.md`
5. **Relatório de Otimização:** `outputs/RA4/relatorios/otimizacao_tac.md`

**Relatórios Semânticos:**
1. **Árvore Atribuída (Markdown):** `outputs/RA3/relatorios/arvore_atribuida.md`
2. **Julgamento de Tipos:** `outputs/RA3/relatorios/julgamento_tipos.md`
3. **Tabela de Símbolos:** `outputs/RA3/relatorios/tabela_simbolos.md`
4. **Erros Semânticos:** `outputs/RA3/relatorios/erros_sematicos.md` (se houver)

**Assembly e Firmware (se AVR Toolchain instalado):**
1. **Assembly AVR:** `outputs/RA4/<nome>.s` (criado manualmente)
2. **Executável Linkado:** `outputs/RA4/<nome>.elf`
3. **Firmware Arduino:** `outputs/RA4/<nome>.hex`
...
**Descrição:** Calcula 8! (fatorial de 8) usando loop WHILE com múltiplas instruções no corpo.

**Algoritmo:**
- COUNTER = 1, RESULT = 1, LIMIT = 8 (inicialização)
- Loop WHILE enquanto COUNTER ≤ LIMIT:
  1. RESULT = RESULT * COUNTER (multiplicação acumulada)
  2. COUNTER = COUNTER + 1 (incremento)
- Resultado final: 8! = 40,320
...
**Valor Final:** `RESULT = 40,320` (8! correto) 
...
**Descrição:** Cálculo da série de Taylor para **cos(x)** com x=0.5 radiano usando loop WHILE.
...
**Algoritmo:**
- X_VAL = 0.5, TERM1 = 1.0, COUNTER = 1
- Loop WHILE (executa 1 vez, COUNTER ≤ 1):
  - **TERMO 2:** TERM2 = -(X_VAL²) / 2! = -0.125
  - **TERMO 3:** TERM3 = (X_VAL⁴) / 4! ≈ 0.002604
  - **TERMO 4:** TERM4 = -(X_VAL⁶) / 6! ≈ -0.000022
  - **SOMA:** RESULT_COS = TERM1 + TERM2 + TERM3 + TERM4 ≈ 0.877582
...
TERM1 = 1.0
TERM2 = -0.25/2.0 = -0.125
TERM3 = 0.0625/24.0 ≈ 0.002604
TERM4 = -0.015625/720.0 ≈ -0.000022
```

**Valor Final:** `FINAL_COS ≈ 0.877582`

**Valor matemático exato:** cos(0.5) = 0.877583... (erro < 0.001%) 
